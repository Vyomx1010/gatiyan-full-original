...
// Import OlaMaps using named export (or adjust if needed)
...
const LiveTracking = ({ sourceCoords, destinationCoords }) => {
  ...
  useEffect(() => {
    ...
    if (mapRef.current && !map) {
      // Initialize the map using OlaMaps from the npm package
      const olaMapsInstance = new OlaMaps({ apiKey });
      // Note: Ola Maps SDK expects center as [lng, lat]
  ...
  return <div ref={mapRef} style={{ width: "100%", height: "100%" }} className="z-10" />;
};
